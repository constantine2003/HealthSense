...
import {
  FaArrowLeft, FaSearch, FaFilter, FaTimes, FaCalendarAlt,
  FaSortAmountDown, FaSortAmountUp, FaChevronDown, FaDownload,
  FaShieldAlt, FaCheckCircle, FaExclamationTriangle, FaHeartbeat
} from "react-icons/fa";
...
interface CheckupRecord {
  ...
  oxygen: string; temp: string; height: string; weight: string; bmi: string; bp: string; heart_rate: string;
}

// ─── PDF ──────────────────────────────────────────────────────────────────────
let jsPDFLib: typeof import("jspdf")["default"] | null = null;
async function preloadPDFLibs() { if (!jsPDFLib) { const { default: jsPDF } = await import("jspdf"); jsPDFLib = jsPDF; } }
...
async function exportToPDF(record: CheckupRecord, language: "English" | "Tagalog", units: "metric" | "imperial") {
  ...
  doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.text("Health Checkup Report", MARGIN, 20); doc.text(`Generated: ${new Date().toLocaleString()}`, MARGIN, 26);
...
const DetailModal: React.FC<{
  record: CheckupRecord; language: "English" | "Tagalog"; units: "metric" | "imperial";
  lang: any; onClose: () => void; getHealthData: (r: CheckupRecord) => any[];
  ...
  return (
    <div className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-8 bg-[#001b2e]/70 backdrop-blur-xl animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-white rounded-4xl border border-[#d0e8f0] shadow-2xl shadow-[#0a4d61]/20 overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-6 duration-200">
        <div className="h-1.5 bg-linear-to-r from-[#139dc7] via-[#34A0A4] to-[#0a4d61] shrink-0" />
        <div className="shrink-0 px-6 sm:px-8 py-5 flex items-start justify-between border-b border-[#139dc7]/10">
          <div>
            <p className="text-[9px] font-black text-[#139dc7] uppercase tracking-widest mb-0.5">{lang.diagnosticSummary}</p>
            <h2 className="text-xl sm:text-2xl font-black text-[#0a4d61] leading-tight">{lang.reportTitle}</h2>
          ...
          <button onClick={onClose} className="w-9 h-9 rounded-xl bg-slate-100 text-slate-400 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all active:scale-90 shrink-0">
            <FaTimes size={13} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 sm:px-8 py-5 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#139dc7]/20 [&::-webkit-scrollbar-thumb]:rounded-full">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            {getHealthData(record).map((data, i) => (
              <div key={i} className="group flex items-center gap-4 p-4 bg-slate-50/80 rounded-2xl border border-[#139dc7]/5 hover:bg-white hover:border-[#139dc7]/20 hover:shadow-sm transition-all">
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0 transition-transform group-hover:scale-110 ${data.type === "success" ? "bg-green-100 text-green-600" : data.type === "warning" ? "bg-orange-100 text-orange-600" : "bg-red-100 text-red-600"}`}>{data.icon}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-[9px] font-black uppercase text-[#139dc7]/40 tracking-widest mb-0.5">{data.title}</p>
                  ...
                  <div className={`mt-1 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${data.type === "success" ? "bg-green-500/10 text-green-600" : data.type === "warning" ? "bg-orange-500/10 text-orange-600" : "bg-red-500/10 text-red-600"}`}>
          ...
          <div className="border-t border-[#139dc7]/10 pt-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="font-black text-[#0a4d61] flex items-center gap-2 text-sm sm:text-base"><FaShieldAlt className="text-[#139dc7]" />{lang.insightsTitle}</h3>
                <p className="text-[9px] text-[#139dc7]/40 font-bold uppercase tracking-widest mt-0.5">{lang.insightsSubtitle}</p>
              </div>
              {conditions.length > 0 && (
                <div className="flex items-center gap-1.5 shrink-0">
                  {highCount > 0 && <span className="flex items-center gap-1 text-[9px] font-black uppercase px-2.5 py-1 bg-red-100 text-red-700 rounded-full border border-red-200"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />{highCount} {lang.riskLabels.high}</span>}
                  {modCount > 0 && <span className="flex items-center gap-1 text-[9px] font-black uppercase px-2.5 py-1 bg-amber-100 text-amber-700 rounded-full border border-amber-200"><span className="w-1.5 h-1.5 rounded-full bg-amber-400" />{modCount} {lang.riskLabels.moderate}</span>}
                </div>
              )}
            </div>
            {conditions.length === 0 ? (
              <div className="bg-green-50 border border-green-200 rounded-2xl p-5 flex items-center gap-4">
                <div className="w-10 h-10 bg-green-100 rounded-xl flex items-center justify-center shrink-0"><FaCheckCircle className="text-green-500 text-lg" /></div>
                <div><p className="font-black text-green-700 text-sm">{lang.allClear}</p><p className="text-green-600/70 text-xs mt-0.5">{lang.allClearDesc}</p></div>
              </div>
            ) : (
              <div className="space-y-2">
                {conditions.map((cond, i) => {
                  const cfg = riskConfig[cond.risk]; const isOpen = expandedCondition === i;
                  ...
                  return (
                    <div key={i} className={`${cfg.bg} ${cfg.border} border rounded-2xl overflow-hidden`}>
                      <button onClick={() => setExpandedCondition(isOpen ? null : i)} className="w-full flex items-center gap-3 p-3.5 text-left">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${cfg.bg} border ${cfg.border}`}>{cfg.icon}</div>
                        <div className="flex-1 min-w-0">
                          <p className="font-black text-[#0a4d61] text-xs sm:text-sm leading-tight truncate">{name}</p>
                          <div className="flex flex-wrap items-center gap-1 mt-0.5">
                            {cond.relatedVitals.map(v => <span key={v} className="text-[7px] font-black uppercase px-1.5 py-0.5 bg-white/70 text-[#139dc7] rounded-full border border-[#139dc7]/20">{v}</span>)}
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          <span className={`text-[7px] font-black uppercase px-2 py-0.5 rounded-full hidden sm:inline ${cfg.badge}`}>{lang.riskLabels[cond.risk]}</span>
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-transform duration-200 ${isOpen ? "bg-[#0a4d61]/10 rotate-180" : "bg-white/50"}`}><FaChevronDown size={9} className="text-[#0a4d61]/60" /></div>
                        </div>
                      </button>
                      {isOpen && <div className="px-4 pb-4"><div className="h-px bg-white/80 mb-3" /><div className="flex gap-2.5"><span className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${cfg.dot}`} /><p className="text-xs text-[#0a4d61]/80 leading-relaxed font-medium">{explanation}</p></div></div>}
                    </div>
                  );
                })}
              </div>
            )}
            <div className="mt-4 flex items-start gap-2"><FiInfo className="text-[#139dc7]/30 shrink-0 mt-0.5" size={11} /><p className="text-[8px] text-[#139dc7]/40 font-medium leading-relaxed">{lang.disclaimer}</p></div>
          </div>
          <div className="mt-5 pt-4 border-t border-[#139dc7]/10 flex flex-col sm:flex-row gap-3 justify-between items-center pb-1">
            <p className="text-[9px] font-bold text-[#139dc7]/30 uppercase tracking-[0.2em]">{lang.verified}</p>
            <button onClick={async () => { setExporting(true); try { await exportToPDF(record, language, "metric"); } catch (err) { alert(`PDF export failed: ${(err as Error).message}`); } finally { setExporting(false); } }} disabled={exporting}
              ...
              {exporting ? <><span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />{lang.exporting}</> : <><FaDownload size={11} />{lang.exportPDF}</>}
            </button>
          </div>
        </div>
      </div>
...
const History: React.FC = () => {
  ...
  const content = {
    English: {
      back: "Back to Dashboard", title: "Checkup History", subtitle: "Your complete health record timeline.",
      searchPlaceholder: "Search by date (e.g. March 2025)...", filter: "Filter", noRecords: "No records match your filters.",
      ...
      allMonths: "All Months", newestFirst: "Newest First", oldestFirst: "Oldest First", clearFilters: "Clear All",
      ...
      relatedVitals: "Related Vitals", disclaimer: "For informational purposes only. Not a substitute for medical advice.",
    ...
    Tagalog: {
      back: "Bumalik sa Dashboard", title: "Kasaysayan ng Checkup", subtitle: "Kumpletong timeline ng iyong medikal na rekord.",
      searchPlaceholder: "Maghanap gamit ang petsa (hal. Marso 2025)...", filter: "I-filter", noRecords: "Walang record na tumutugma sa filter.",
      ...
      allMonths: "Lahat ng Buwan", newestFirst: "Pinakabago Muna", oldestFirst: "Pinakamatanda Muna", clearFilters: "I-clear Lahat",
      ...
      relatedVitals: "Kaugnay na Vital Signs", disclaimer: "Para sa impormasyon lamang. Hindi kapalit ng medikal na payo.",
      vitals: { spo2: "SpO2", temp: "Temperatura", height: "Tangkad", weight: "Timbang", bmi: "BMI", bp: "BP", hr: "Heart Rate" },
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data, error } = await supabase.from("health_checkups")
  ...
  const activeFilterCount = useMemo(() => (monthFilter !== "all" ? 1 : 0) + (sortDir !== "newest" ? 1 : 0) + (riskFilter !== "all" ? 1 : 0), [monthFilter, sortDir, riskFilter]);
  ...
  const filteredData = useMemo(() => {
    ...
    return result;
  }, [historyData, searchQuery, monthFilter, riskFilter, sortDir]);

  const clearFilters = () => { setMonthFilter("all"); setSortDir("newest"); setSearchQuery(""); setRiskFilter("all"); };

  const getHealthData = (record: CheckupRecord) => {
    const l = content[language]; const isMetric = units === "metric"; const healthData = [];
    const s = Number(record.oxygen); let spo2Status = l.status.normal, spo2Type: StatusType = "success";
    ...
    const t = Number(record.temp), displayTemp = isMetric ? t : (t*9/5)+32; let tempStatus = l.status.normal, tempType: StatusType = "success";
    if (t < 30) { tempStatus = l.status.hypo; tempType = "danger"; } else if (t <= 37.5) { tempStatus = l.status.normal; } else if (t <= 39) { tempStatus = l.status.fever; tempType = "warning"; } else { tempStatus = l.status.highFever; tempType = "danger"; }
    ...
    const rawH = Number(record.height), heightM = rawH/100, displayH = isMetric ? heightM : heightM*39.3701; let heightStatus = l.status.average, heightType: StatusType = "success";
    ...
    const w = Number(record.weight), displayWeight = isMetric ? w : w*2.20462, b = Number(record.bmi); let bmiStatus = l.status.normal, bmiType: StatusType = "success";
    if (b < 18.5) { bmiStatus = l.status.under; bmiType = "warning"; } else if (b < 25) { bmiStatus = l.status.normal; } else if (b < 30) { bmiStatus = l.status.over; bmiType = "warning"; } else { bmiStatus = l.status.obese; bmiType = "danger"; }
  ...
  const groupedByMonth = useMemo(() => {
    const groups: { label: string; records: CheckupRecord[] }[] = [];
    filteredData.forEach(record => {
      const label = record.rawDate.toLocaleDateString("en-US", { month: "long", year: "numeric" });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#eaf4ff] font-['Lexend']">
        <div className="text-center">
          ...
          <h2 className="text-xl font-black text-[#139dc7]">Loading History</h2>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen h-screen flex flex-col bg-[linear-gradient(120deg,#eaf4ff_0%,#cbe5ff_40%,#b0d0ff_70%,#9fc5f8_100%)] font-['Lexend'] overflow-hidden">
      ...
      <header className="shrink-0 w-full px-4 sm:px-8 lg:px-16 py-5 flex justify-between items-center">
        <button onClick={() => navigate("/dashboard")} className="flex items-center gap-2 text-[#139dc7] font-bold hover:gap-3 transition-all active:scale-95 text-sm">
          <FaArrowLeft /> {lang.back}
        </button>
      </header>

      {/* PAGE TITLE + CONTROLS — centered max-w container */}
      <div className="shrink-0 w-full max-w-6xl mx-auto px-6 sm:px-10 pb-5">
        <h1 className="text-4xl lg:text-5xl font-black text-[#139dc7] mb-1">{lang.title}</h1>
        <p className="text-[#139dc7]/50 text-sm lg:text-base mb-5">{lang.subtitle}</p>

        {/* Search + controls row */}
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          {/* Search bar — full width */}
          <div className="flex-1 relative">
            <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-[#139dc7]/40 pointer-events-none" />
            <input type="text" placeholder={lang.searchPlaceholder} value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="w-full h-12 lg:h-14 bg-white/60 backdrop-blur-md border-2 border-white/60 focus:border-[#139dc7]/40 rounded-2xl pl-11 pr-5 outline-none text-[#139dc7] font-bold text-sm lg:text-base placeholder:text-[#139dc7]/30 transition-all" />
            {searchQuery && <button onClick={() => setSearchQuery("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#139dc7]/40 hover:text-[#139dc7] transition-colors"><FaTimes size={14} /></button>}
          </div>
          {/* Sort + Filter — side by side on mobile too */}
          <div className="flex gap-2 sm:contents">
          <button onClick={() => setSortDir(d => d === "newest" ? "oldest" : "newest")}
            className="flex-1 sm:flex-none h-12 lg:h-14 px-3 sm:px-4 lg:px-5 bg-white/60 backdrop-blur-md border-2 border-white/60 hover:border-[#139dc7]/30 rounded-2xl text-[#139dc7] flex items-center justify-center gap-2 font-bold text-xs lg:text-sm transition-all hover:bg-white active:scale-95">
            {sortDir === "newest" ? <FaSortAmountDown size={14} /> : <FaSortAmountUp size={14} />}
            <span className="hidden sm:inline">{sortDir === "newest" ? lang.newestFirst : lang.oldestFirst}</span>
            <span className="sm:hidden text-[10px]">{sortDir === "newest" ? "Newest" : "Oldest"}</span>
          </button>
          <div className="flex-1 sm:flex-none relative" ref={filterPanelRef}>
            <button onClick={() => setShowFilterPanel(p => !p)}
              className={`w-full h-12 lg:h-14 px-3 sm:px-5 border-2 rounded-2xl flex items-center justify-center gap-2 font-bold text-xs lg:text-sm transition-all active:scale-95
                ${showFilterPanel || activeFilterCount > 0 ? "bg-[#139dc7] border-[#139dc7] text-white shadow-lg shadow-[#139dc7]/20" : "bg-white/60 backdrop-blur-md border-white/60 hover:border-[#139dc7]/30 text-[#139dc7] hover:bg-white"}`}>
              <FaFilter size={13} />
            ...
            {showFilterPanel && (
              <div className="absolute right-0 top-full mt-2 w-72 bg-white/95 backdrop-blur-xl rounded-[28px] border border-white shadow-2xl shadow-[#139dc7]/10 z-50 p-6 animate-in fade-in zoom-in-95 duration-150">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button onClick={clearFilters} className="text-[9px] font-black text-[#139dc7]/50 hover:text-[#139dc7] transition-colors">{lang.clearFilters}</button>
                </div>
                <div className="mb-5">
                  <label className="text-[9px] font-black text-[#139dc7]/50 uppercase tracking-widest block mb-2">{lang.filterMonth}</label>
                  <div className="relative">
                    <select value={monthFilter} onChange={e => setMonthFilter(e.target.value)} className="w-full appearance-none bg-slate-50 border-none rounded-xl px-4 py-2.5 text-[11px] font-bold text-[#0a4d61] outline-none cursor-pointer">
                      <option value="all">{lang.allMonths}</option>
                      {availableMonths.map(m => { const [yr, mo] = m.split("-"); return <option key={m} value={m}>{new Date(Number(yr), Number(mo)-1).toLocaleDateString("en-US", { month: "long", year: "numeric" })}</option>; })}
                    </select>
                ...
                <div>
                  ...
                  <div className="flex gap-1.5">
                    {([{ key: "newest", label: lang.newestFirst, icon: <FaSortAmountDown size={11} /> }, { key: "oldest", label: lang.oldestFirst, icon: <FaSortAmountUp size={11} /> }] as { key: SortDir; label: string; icon: React.ReactNode }[]).map(opt => (
                      <button key={opt.key} onClick={() => setSortDir(opt.key)}
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${sortDir === opt.key ? "bg-[#139dc7] text-white shadow-md shadow-[#139dc7]/20" : "bg-slate-50 text-[#0a4d61]/60 hover:bg-slate-100"}`}>
                        {opt.icon} {opt.label}
                      </button>
                    ))}
                  </div>
                ...
                {/* Risk Level Filter */}
                <div className="mt-5">
                  <label className="text-[9px] font-black text-[#139dc7]/50 uppercase tracking-widest block mb-2">
                    {language === "English" ? "Risk Level" : "Antas ng Panganib"}
                  </label>
                  <div className="grid grid-cols-2 gap-1.5">
                    {([
                      { key: "all",      label: language === "English" ? "All Records" : "Lahat",    dot: null,        active: "bg-[#139dc7] text-white" },
                      { key: "none",     label: language === "English" ? "No Risk" : "Walang Panganib", dot: "bg-emerald-400", active: "bg-emerald-500 text-white" },
                      { key: "low",      label: language === "English" ? "Low Risk" : "Mababa",       dot: "bg-sky-400",     active: "bg-sky-500 text-white" },
                      { key: "moderate", label: language === "English" ? "Moderate" : "Katamtaman",   dot: "bg-amber-400",   active: "bg-amber-500 text-white" },
                      { key: "high",     label: language === "English" ? "High Risk" : "Mataas",      dot: "bg-red-400",     active: "bg-red-500 text-white" },
                    ] as { key: "all"|"none"|"low"|"moderate"|"high"; label: string; dot: string|null; active: string }[]).map(opt => (
                      <button key={opt.key} onClick={() => setRiskFilter(opt.key)}
                        className={`flex items-center justify-center gap-1.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all
                          ${opt.key === "all" ? "col-span-2" : "col-span-1"}
                          ${riskFilter === opt.key ? opt.active : "bg-slate-50 text-[#0a4d61]/60 hover:bg-slate-100"}`}>
                        {opt.dot && <span className={`w-2 h-2 rounded-full ${riskFilter === opt.key ? "bg-white/70" : opt.dot}`} />}
                        {opt.label}
                      </button>
                    ))}
                  </div>
        ...
        {activeFilterCount > 0 && (
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {sortDir !== "newest" && (
              <span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-wider px-3 py-1.5 bg-[#139dc7]/10 text-[#139dc7] rounded-full border border-[#139dc7]/20">
                {lang.oldestFirst}<button onClick={() => setSortDir("newest")} className="hover:text-[#0a4d61] ml-1"><FaTimes size={8} /></button>
              </span>
            )}
            {riskFilter !== "all" && (
              <span className={`flex items-center gap-1.5 text-[9px] font-black uppercase tracking-wider px-3 py-1.5 rounded-full border
                ${riskFilter === "none" ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                  riskFilter === "low" ? "bg-sky-50 text-sky-700 border-sky-200" :
                  riskFilter === "moderate" ? "bg-amber-50 text-amber-700 border-amber-200" :
                  "bg-red-50 text-red-700 border-red-200"}`}>
                <span className={`w-1.5 h-1.5 rounded-full ${riskFilter === "none" ? "bg-emerald-400" : riskFilter === "low" ? "bg-sky-400" : riskFilter === "moderate" ? "bg-amber-400" : "bg-red-500"}`} />
                {riskFilter === "none" ? (language === "English" ? "No Risk" : "Walang Panganib") :
                 riskFilter === "low" ? (language === "English" ? "Low Risk" : "Mababang Panganib") :
                 riskFilter === "moderate" ? (language === "English" ? "Moderate" : "Katamtamang Panganib") :
                 (language === "English" ? "High Risk" : "Mataas na Panganib")}
                <button onClick={() => setRiskFilter("all")} className="hover:opacity-70 ml-0.5"><FaTimes size={8} /></button>
              </span>
            )}
            <button onClick={clearFilters} className="text-[9px] font-black text-red-400 hover:text-red-600 uppercase tracking-wider">{lang.clearFilters}</button>
          </div>
        )}

        {/* Record count */}
        <p className="text-[10px] lg:text-xs font-black text-[#139dc7]/40 uppercase tracking-widest mt-3">
          {filteredData.length} {filteredData.length === 1 ? "record" : "records"}{(searchQuery || activeFilterCount > 0) && ` of ${historyData.length}`}
        </p>
      </div>
        ...
        <div className="w-full max-w-6xl mx-auto px-6 sm:px-10">
          {filteredData.length === 0 ? (
            <div className="bg-white/40 rounded-3xl p-16 text-center border border-white/40 flex flex-col items-center gap-4">
              <div className="w-14 h-14 rounded-2xl bg-[#139dc7]/10 flex items-center justify-center"><FaSearch className="text-[#139dc7]/40" size={20} /></div>
              ...
              {groupedByMonth.map((group, gi) => (
                <div key={gi}>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <span className="text-[10px] lg:text-sm font-black text-[#139dc7] uppercase tracking-[0.2em]">{group.label}</span>
                    <div className="flex-1 h-px bg-[#139dc7]/15" />
                    <span className="text-[9px] font-bold text-[#139dc7]/30">{group.records.length} {group.records.length === 1 ? "visit" : "visits"}</span>
                  </div>

                  {/* Records */}
                  <div className="space-y-3 pl-3 border-l-2 border-[#139dc7]/10">
                    {group.records.map((record, index) => {
                      const healthData = getHealthData(record);
                      const conditions = analyzeHealth(record);
                      const highRisk = conditions.filter(c => c.risk === "high").length;
                      const modRisk  = conditions.filter(c => c.risk === "moderate").length;
                      const allNormal = conditions.length === 0;
                      return (
                        <div key={index} className="relative ml-4">
                          {/* Timeline dot */}
                          <div className={`absolute -left-6.25 top-6 w-3 h-3 rounded-full border-2 border-white shadow-sm ${allNormal ? "bg-emerald-400" : highRisk > 0 ? "bg-red-400" : "bg-amber-400"}`} />

                          <div className="bg-white/70 backdrop-blur-xl border border-white rounded-3xl p-4 sm:p-5 lg:p-8 hover:shadow-lg hover:shadow-[#139dc7]/8 hover:-translate-y-0.5 transition-all group">

                            {/* Date + time row */}
                            <div className="flex items-center gap-3 mb-3">
                              <div className="w-8 h-8 lg:w-11 lg:h-11 bg-[#139dc7]/10 rounded-xl flex items-center justify-center text-[#139dc7] shrink-0">
                                <FaCalendarAlt size={13} />
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="font-black text-[#0a4d61] text-sm sm:text-base lg:text-xl leading-tight truncate">{record.date}</p>
                                <p className="text-[9px] lg:text-xs text-[#139dc7]/50 font-black uppercase tracking-widest mt-0.5">{lang.checkedAt} {record.time}</p>
                              </div>
                            </div>

                            {/* Risk badges row */}
                            <div className="flex items-center gap-2 mb-3 flex-wrap">
                              {allNormal && <span className="flex items-center gap-1 text-[8px] font-black uppercase px-2 py-1 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-200"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />Normal</span>}
                              {highRisk > 0 && <span className="flex items-center gap-1 text-[8px] font-black uppercase px-2 py-1 bg-red-50 text-red-700 rounded-lg border border-red-200"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />{highRisk} High Risk</span>}
                              {modRisk > 0 && <span className="flex items-center gap-1 text-[8px] font-black uppercase px-2 py-1 bg-amber-50 text-amber-700 rounded-lg border border-amber-200"><span className="w-1.5 h-1.5 rounded-full bg-amber-400" />{modRisk} Moderate</span>}
                            </div>

                            {/* Vitals mini grid — 4 on mobile, 7 on desktop */}
                            <div className="grid grid-cols-4 lg:grid-cols-7 gap-1.5 sm:gap-2 mb-3">
                              {healthData.map((stat, i) => (
                                <div key={i} className="bg-white/60 border border-white p-2 sm:p-2.5 rounded-xl lg:rounded-2xl group-hover:bg-white transition-colors">
                                  <p className="text-[7px] lg:text-[9px] font-black text-[#139dc7] uppercase mb-1 tracking-tight opacity-50 truncate">{stat.title}</p>
                                  <p className="font-bold text-[#0a4d61] leading-none text-xs sm:text-sm lg:text-base">
                                    {stat.value}{stat.unit && <span className="text-[6px] sm:text-[7px] ml-0.5 opacity-50 font-medium">{stat.unit}</span>}
                                  </p>
                                </div>
                              ))}
                            </div>
                            ...
                            <button onClick={() => setSelectedRecord(record)}
                              className="w-full sm:w-auto px-4 lg:px-6 py-2.5 lg:py-3 bg-[#139dc7] text-white rounded-xl font-black text-xs lg:text-sm uppercase tracking-widest hover:bg-[#0a4d61] transition-all active:scale-95 shadow-md shadow-[#139dc7]/20 flex items-center justify-center gap-2">
                              {lang.viewDetails}
                            </button>
                          </div>
                        </div>
