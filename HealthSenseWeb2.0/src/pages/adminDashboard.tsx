...
const AdminDashboard = () => {
  ...
  /**
   * PASSWORD CHANGE via SQL RPC
   * Mirrors: UPDATE auth.users SET encrypted_password = crypt($1, gen_salt('bf')) WHERE id = $2
   *
   * You need to create this function in Supabase SQL Editor first:
   *
   * CREATE OR REPLACE FUNCTION admin_change_password(user_id uuid, new_password text)
   * RETURNS void
   * LANGUAGE plpgsql
   * SECURITY DEFINER
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setPasswordMsg({ text: err.message ?? "Unknown error.", ok: false });
    } finally {
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-slate-50/80 rounded-[40px] p-8 border border-slate-100 flex flex-col h-full ring-1 ring-black/5">
                  ...
                  <p className="text-[11px] text-[#0a4d61]/60 mb-8 font-medium leading-relaxed bg-white/50 p-4 rounded-2xl border border-white">
                    Directly updates <code className="text-[#139dc7]">auth.users.encrypted_password</code> via bcrypt — same as the SQL script. Input is <b>unmasked</b> for admin verification.
                  </p>
                  ...
                  <div className="space-y-4 mt-auto">
                    ...
                    {passwordMsg && (
                      <p className={`text-[11px] font-black px-4 py-3 rounded-2xl ${passwordMsg.ok ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-500'}`}>
                        {passwordMsg.text}
                      </p>
                    )}
