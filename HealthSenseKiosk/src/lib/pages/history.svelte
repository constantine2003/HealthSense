<script lang="ts">
  ...
  const getStatus = (val: any, type: string) => {
    if (val === undefined || val === null || val === 0) return { status: "N/A", color: "#94a3b8" };
    if (type === 'spo2') {
  ...
  const getBMIInfo = (item: any) => {
    let b: number;
    if (item.bmi && item.bmi !== 0) {
      b = parseFloat(item.bmi);
    } else if (item.weight && item.height && item.height > 0) {
      b = parseFloat((item.weight / (item.height * item.height)).toFixed(1));
    } else {
      return { bmi: "N/A", status: "N/A", color: "#94a3b8" };
    }
    ...
    return { bmi: b.toFixed(1), status, color };
  };
